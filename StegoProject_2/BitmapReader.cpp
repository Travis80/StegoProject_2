...
void printHelp()
{
	...
	printf("Where 'source filename' is the name of the bitmap message file.\n");
	printf("Where 'target filename' is the name of the bitmap cover file.\n");
...
int horizontalComplexity(unsigned char lsbM[]) {
	int complexityCnt = 0;
	int i;
	for(i = 0; i < 64; i++) {
		if ((i + 1) % 8 == 0) {
...
void getBlockByBit(int bit, unsigned char lsbM[], unsigned char * pPixelVal) {
	int i;
	for (i = 0; i < 64; i++) {
		lsbM[i] = *pPixelVal & 1; //AND to get the LSB
		pPixelVal++;
	}
}

int verticalComplexity(unsigned char lsbM[]) {
	int complexityCnt = 0;
	int j;
	int i;
	for (i = 0; i < 8; i++) {
		for (j = 0; j < 63 - (15-i); j += 8) {
			if (lsbM[j] == lsbM[j + 8]) {
				complexityCnt++;
			}
		}
...
void main(int argc, char *argv[])
{
	...
	pTgtFile = readFile(argv[2], &tgtFileSize); // holds beginning of file
	...
	unsigned char lsbM[64];
	unsigned char * pPixelVal = pTgtData;
	printf("%p\n", * pPixelVal);
	int i;
	int k;
	float threshold = 0.3;
	int thresh = threshold * 56;
	int iHcomplexityCnt = 0;
	int iVcomplexityCnt = 0;
	for (k = 0; k < 4; k++) {
		getBlockByBit(1, lsbM, pPixelVal);
		pPixelVal += 64; // jump by 64 bytes
		iHcomplexityCnt += horizontalComplexity(lsbM);
		iVcomplexityCnt = verticalComplexity(lsbM);
		printf("H = %d\n", iHcomplexityCnt);
		printf("V = %d\n", iVcomplexityCnt);
		if (iHcomplexityCnt > thresh && iVcomplexityCnt > thresh) {
			printf("This block meets the threshold\n");
			// hide
			int lm;
			//change
			unsigned char * pMsgVal = pSrcData;
			printf("%p\n", *pMsgVal);
			unsigned char msgM[64];
			for (lm = 0; lm < 64; lm+=8) {
				msgM[lm] = *pMsgVal & 1;
				*pTgtData = *pTgtData | msgM[lm];
				printf("msg=%p\n", msgM[lm]);
				printf("data=%p\n", *pTgtData);
				pTgtData++;
				msgM[lm + 1] = *pMsgVal & 2 >> 1;
				*pTgtData = *pTgtData | msgM[lm];
				printf("msg=%p\n", msgM[lm + 1]);
				printf("data=%p\n", *pTgtData);
				pTgtData++;
				...
				msgM[lm + 3] = *pMsgVal & 8;
				printf("%p\n", msgM[lm + 3]);
				msgM[lm + 4] = *pMsgVal & 16;
				printf("%p\n", msgM[lm + 4]);
				msgM[lm + 5] = *pMsgVal & 32;
				printf("%p\n", msgM[lm + 5]);
				msgM[lm + 6] = *pMsgVal & 64;
				printf("%p\n", msgM[lm + 6]);
				msgM[lm + 7] = *pMsgVal & 128;
				printf("%p\n", msgM[lm + 7]);*/
				pMsgVal += 8;
			}
		}
